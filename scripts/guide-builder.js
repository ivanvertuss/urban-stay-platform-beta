...
async function loadGuideProperties(){if(!DB)return;try{const {data:{user},error:userError}=await DB.auth.getUser();if(userError||!user)return;const {data,error}=await DB.from('properties').select('id,name,city,country,status,review_status,updated_at').eq('owner_id',user.id).order('created_at',{ascending:false});if(error)throw error;const localMap=new Map(localProps().map(p=>[String(p.id),p]));serverProps=(data||[]).map(row=>{const old=localMap.get(String(row.id))||{};const uiStatus=row.review_status==='approved'?(old.status==='active'||old.status==='inactive'?old.status:'approved'):(row.review_status==='rejected'?'changes_requested':'pending_review');return {...old,id:row.id,name:row.name,city:row.city||'',cityLabel:[row.city,row.country].filter(Boolean).join(', '),country:row.country||'',status:uiStatus,reviewStatus:row.review_status||'pending',updatedAt:row.updated_at||old.updatedAt}})}catch(e){console.warn('Urban Stay guide property load failed',e);serverProps=[]}}
